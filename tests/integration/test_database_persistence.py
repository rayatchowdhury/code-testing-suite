...
from src.app.persistence.database.models import TestResult, Session, ProjectData, FilesSnapshot
from src.app.persistence.database.services.files_snapshot_service import FilesSnapshotService
...
class TestFilesSnapshotPersistence:
    ...
    def test_snapshot_multi_language_detection(self, temp_db, workspace_with_files):
        ...
        retrieved_languages = set(
            file_info['language'] 
            for file_info in retrieved_snapshot.files.values()
        )
...
class TestFilteringAndQuerying:
    ...
    def test_filter_by_status(self, temp_db):
        ...
        for i in range(2):
            result = TestResultBuilder() \
                .with_results(passed=7, failed=3) \
                .build()
            temp_db.save_test_result(result)
...
class TestErrorHandling:
    ...
    def test_handles_corrupted_json_in_snapshot(self, temp_db):
        ...
        result = TestResultBuilder() \
            .with_files_snapshot('{"invalid json}') \
            .build()
