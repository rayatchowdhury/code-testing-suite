...
class TestBasicBenchmarking:
    ...
    def test_basic_benchmark_workflow(self, cpp_benchmarker_workspace, qtbot):
        ...
        def on_test_completed(test_name, test_num, passed, exec_time, memory, memory_passed, 
        ...
        if not all_tests_done[0]:
            ...
            for test in completed_tests:
                print(f"Test {test['test_num']}: passed={test['passed']}, exec_time={test['exec_time']}, memory={test['memory']}")
        ...
        for test in completed_tests:
            ...
            assert test['memory'] >= 0, "Memory usage should be tracked (can be 0 for very fast tests)"
...
class TestParallelExecution:
    ...
    def test_parallel_execution(self, cpp_benchmarker_workspace, qtbot):
        ...
        benchmarker.run_benchmark_test(test_count=10, time_limit=2000, memory_limit=256, max_workers=4)
        qtbot.waitUntil(lambda: len(all_tests_done) > 0, timeout=30000)
        ...
        assert total_time < 15.0, f"Parallel execution should complete in reasonable time (took {total_time:.2f}s)"
...
class TestCompleteWorkflow:
    ...
    def test_complete_benchmark_workflow(self, cpp_benchmarker_workspace, temp_db, qtbot):
        ...
        assert 'test_type' in snapshot or len(files_dict) > 0, "Should have valid snapshot structure"
