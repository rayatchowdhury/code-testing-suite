...
class TestComparisonWorkerInitialization:
    ...
    def test_init_sets_executables(self, temp_workspace):
        ...
        worker = ComparisonTestWorker(str(temp_workspace), executables, test_count=5)
...
class TestComparisonWorkerExecution:
    ...
    def test_runs_three_stage_comparison(self, mock_psutil, mock_popen, temp_workspace):
        ...
        correct_proc.communicate.return_value = (
            "test_output",
            "",
        )  # Same output = pass
...
class TestComparisonWorkerSignals:
    ...
    def test_emits_test_completed_with_correct_data(
        ...
        assert (
            len(signal_data) == 7
        )  # test_num, passed, input, correct_output, test_output, time, memory
