...
class TestBenchmarkWorkerExecution:
    ...
    @patch('subprocess.run')
    @patch('psutil.Process')
    def test_fails_on_memory_limit_exceeded(self, mock_psutil, mock_run, mock_popen, temp_workspace):
...
class TestBenchmarkWorkerSignals:
    ...
    @patch('subprocess.Popen')
    @patch('psutil.Process')
    def test_emits_test_completed_with_metrics(self, mock_psutil, mock_popen,
        ...
        assert len(signal_data) == 9  # test_name, test_num, passed, time, memory, memory_passed, input, output, test_size
