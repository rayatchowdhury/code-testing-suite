...
from src.app.persistence.database.services.database_maintenance_service import DatabaseMaintenanceService
...
def populated_service(maintenance_service):
    ...
    with maintenance_service.db.get_connection() as conn:
        ...
        cursor.execute(
            ...
            ("old_session", "[]", "", (datetime.now() - timedelta(days=50)).isoformat(), "old_project")
        )
...
class TestCleanupOldData:
    ...
    def test_handles_no_old_data(self, maintenance_service):
        ...
        result = ResultBuilder() \
            .with_timestamp(datetime.now().isoformat()) \
            .build()
        repo.save(result)
    ...
    def test_respects_days_parameter(self, maintenance_service):
        ...
        repo.save(ResultBuilder().with_timestamp((datetime.now() - timedelta(days=5)).isoformat()).build())
        repo.save(ResultBuilder().with_timestamp((datetime.now() - timedelta(days=15)).isoformat()).build())
        repo.save(ResultBuilder().with_timestamp((datetime.now() - timedelta(days=25)).isoformat()).build())
...
class TestVacuumIsolationLevel:
    ...
    def test_cleanup_vacuum_uses_autocommit(self, maintenance_service):
        ...
        # Add old data
        repo.save(ResultBuilder().with_timestamp((datetime.now() - timedelta(days=50)).isoformat()).build())
...
class TestReturnValues:
    ...
    def test_optimize_returns_complete_dict(self, maintenance_service):
        ...
        required_keys = ['size_before_bytes', 'size_after_bytes', 'space_saved_bytes', 
