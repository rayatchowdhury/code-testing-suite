...
from src.app.persistence.database.exceptions import RepositoryError, DatabaseError as DBError
...
class ConcreteRepository(BaseRepository[SampleEntity]):
    ...
    def get_by_id(self, id: int) -> Optional[SampleEntity]:
        ...
        row = self._execute_query(
            "SELECT id, name FROM test_entities WHERE id = ?",
            (id,),
            fetch_one=True
        )
        if row:
    ...
    def get_all(self, limit: int = 100) -> List[SampleEntity]:
        """Get all entities."""
        cursor = self._execute_query(
            f"SELECT id, name FROM test_entities LIMIT ?",
            (limit,)
        )
