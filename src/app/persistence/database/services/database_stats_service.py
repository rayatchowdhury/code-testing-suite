...
class DatabaseStatsService:
    ...
    def get_database_stats(self) -> Dict:
        ...
        with self.db.get_connection() as conn:
            ...
            cursor.execute("SELECT MIN(timestamp) as min_ts, MAX(timestamp) as max_ts FROM test_results")
            result = cursor.fetchone()
            if result:
            ...
            cursor.execute("SELECT MIN(timestamp) as min_ts, MAX(timestamp) as max_ts FROM sessions")
            result = cursor.fetchone()
            if result:
                row_dict = dict(result)
