...
class DetailedResultsWidget(QWidget):
    ...
    def _create_summary_page(self):
        ...
        stats_layout.addWidget(self._create_info_label("Test Type:", self.test_result.test_type.upper()))
        stats_layout.addWidget(self._create_info_label("File:", os.path.basename(self.test_result.file_path)))
        ...
        # Test statistics
        success_rate = (self.test_result.passed_tests / self.test_result.test_count * 100) if self.test_result.test_count > 0 else 0
        stats_layout.addWidget(self._create_info_label("Total Tests:", str(self.test_result.test_count)))
        stats_layout.addWidget(self._create_info_label("✅ Passed:", str(self.test_result.passed_tests), MATERIAL_COLORS['primary']))
        stats_layout.addWidget(self._create_info_label("❌ Failed:", str(self.test_result.failed_tests), MATERIAL_COLORS['error']))
        stats_layout.addWidget(self._create_info_label("Success Rate:", f"{success_rate:.1f}%"))
        stats_layout.addWidget(self._create_info_label("Total Time:", f"{self.test_result.total_time:.3f}s"))
    ...
    def _create_code_page(self):
        ...
        try:
            files_snapshot = json.loads(self.test_result.files_snapshot) if self.test_result.files_snapshot else {}
    ...
    def _create_test_list_page(self, passed_only: bool):
        ...
        # Parse test details
        try:
            test_details = json.loads(self.test_result.test_details) if self.test_result.test_details else []
        except json.JSONDecodeError:
            test_details = []
    ...
    def _create_test_case_widget(self, test: dict, show_errors: bool = False):
        ...
        if show_errors and not passed:
            expected = test.get('output', test.get('expected_output', test.get('correct_output', '')))
    ...
    def _load_to_test(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to parse code files from test result:\n{str(e)}"
            )
        except Exception as e:
            QMessageBox.critical(
                self,
