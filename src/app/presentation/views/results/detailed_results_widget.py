...
class TestCaseDetailWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        status_color = (
            ...
            if self.test_case_data.get("passed", False)
            else MATERIAL_COLORS["error"]
        )
        ...
        if (
            not self.test_case_data.get("passed", False)
            and "mismatch_analysis" in self.test_case_data
        ):
    ...
    def _create_io_tab(self):
        ...
        try:
            ...
            if not self.test_case_data.get("passed", False):
                # Expected output
                if (
                    "correct_output" in self.test_case_data
                    and self.test_case_data["correct_output"]
                ):
    ...
    def _create_mismatch_tab(self):
        ...
        try:
            ...
            if "summary" in mismatch_data and isinstance(
                ...
                try:
                    stats_info.append(
                        f"Character differences: {summary.get('total_char_differences', 0)}"
                    )
                    stats_info.append(
            ...
            if (
                "line_differences" in mismatch_data
                and isinstance(mismatch_data["line_differences"], list)
                and mismatch_data["line_differences"]
            ):
                try:
                    ...
                    for row, diff in enumerate(line_diffs):
                        if isinstance(diff, dict):
                            ...
                            diff_table.setItem(
                                row, 3, QTableWidgetItem(str(diff.get("actual", "")))
                            )
                        else:
                    ...
                    if len(mismatch_data["line_differences"]) > 50:
                        note_label = QLabel(
                            f"Showing first 50 of {len(mismatch_data['line_differences'])} differences"
                        )
            ...
            if "unified_diff" in mismatch_data and mismatch_data["unified_diff"]:
                try:
                    ...
                    if isinstance(mismatch_data["unified_diff"], list):
                        diff_content = "\n".join(
                            str(line) for line in mismatch_data["unified_diff"]
                        )
                    else:
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            try:
                additional_files = {}
                if (
                    hasattr(self.files_data, "additional_files")
...
class DetailedResultsWidget(QWidget):
    ...
    def _setup_error_ui(self, error_message):
        ...
        if self.test_result:
            info_label = QLabel(
                f"Test Result ID: {getattr(self.test_result, 'id', 'Unknown')}\n"
        ...
        if "failed_tests" in analysis_data:
            ...
            if failed_count > 0:
                summary_label = QLabel(
                    f"Found {failed_count} failed test cases with detailed mismatch analysis"
                )
