...
from PySide6.QtWidgets import QPushButton, QVBoxLayout, QWidget, QMessageBox, QFileDialog
...
class ResultsWindow(SidebarWindowBase):
    ...
    def clear_old_data(self):
        ...
        if reply == QMessageBox.Yes:
            self.display_area.db_manager.cleanup_old_data(7)  # Phase 3 (Issue #32): Changed from 30 to 7 days
            self.display_area._load_results()
    ...
    def show_detailed_view(self, test_result):
        ...
        """
        from src.app.presentation.views.results.detailed_results_window import DetailedResultsWidget
    ...
    def _handle_load_to_test(self, load_data: dict):
        ...
        try:
            ...
            print("="*60)
            print(f"Load data contents: {json.dumps({k: (v if k != 'files' else f'{len(v)} files') for k, v in load_data.items()}, indent=2)}")
            ...
            if not show_result:
                ...
                QMessageBox.critical(
                    self,
                    "Error",
                    f"Failed to open {window_name.title()} window."
                )
                return
            ...
            print(f"DEBUG: Target window obtained: {type(target_window).__name__ if target_window else 'None'}")
            ...
            if not target_window:
                ...
                QMessageBox.critical(
                    self,
                    "Error",
                    f"Failed to access {window_name.title()} window."
                )
                return
            ...
            QMessageBox.critical(
                self,
                "Load Error",
                f"Failed to load code files: {str(e)}"
            )
        ...
        try:
            ...
            for file_key, tab_name in file_key_mapping.items():
                ...
                if file_key in files:
                    ...
                    if test_tabs and hasattr(test_tabs, 'switch_language') and hasattr(test_tabs, 'multi_language') and test_tabs.multi_language:
        ...
        try:
            ...
            if hasattr(display_area, 'editor'):
                ...
                if hasattr(editor, 'codeEditor'):
                    ...
                    print(f"      - codeEditor has setPlainText: {hasattr(code_editor, 'setPlainText')}")
                
                # Try to set code content
                if code_editor and hasattr(code_editor, 'setPlainText'):
                    print(f"    - Setting code via codeEditor.setPlainText() ({len(code_content)} chars)")
                    code_editor.setPlainText(code_content)
                    print(f"    - Code set successfully via codeEditor")
                elif hasattr(editor, 'setPlainText'):
                    print(f"    - Setting code with editor.setPlainText() ({len(code_content)} chars)")
