...
from src.app.presentation.shared.design_system.styles.components.config import CONFIG_DIALOG_STYLE
...
class ErrorDialog(QDialog):
    ...
    def _setup_styles(self):
        ...
        from src.app.presentation.shared.design_system.styles.components.config_ui import ERROR_DIALOG_STYLE
...
class ConfigView(QDialog):
    ...
    def _create_editor_section(self):
        ...
        font_family_layout = QHBoxLayout(font_family_widget)
        ...
        self.font_family_combo.addItems(["Consolas", "Courier New", "Monaco", "Fira Code", "Source Code Pro", "JetBrains Mono"])
        self.font_family_combo.setFixedHeight(28)
        self.font_family_combo.setToolTip("Select monospace font for code editor")
        ...
        font_size_label.setFixedWidth(120)
        ...
        self.font_size_spin.setToolTip("Font size in points")
        ...
        # Tab width
        tab_width_widget = QWidget()
        tab_width_layout = QHBoxLayout(tab_width_widget)
        tab_width_layout.setContentsMargins(0, 0, 0, 0)
        tab_width_layout.setSpacing(8)

        tab_width_label = QLabel("Tab width:")
        tab_width_label.setFixedWidth(120)
        ...
        self.tab_width_spin.setFixedHeight(28)
        self.tab_width_spin.setToolTip("Number of spaces per tab")
        tab_width_layout.addWidget(self.tab_width_spin)
        tab_width_layout.addStretch(1)

        layout.addWidget(tab_width_widget)

        # Line wrap
        self.wrap_checkbox = QCheckBox("Enable line wrap")
        self.wrap_checkbox.setToolTip("Wrap long lines to fit in editor window")
        layout.addWidget(self.wrap_checkbox)

        # Bracket matching
        self.bracket_match_checkbox = QCheckBox("Enable bracket matching")
        self.bracket_match_checkbox.setChecked(True)
        self.bracket_match_checkbox.setToolTip("Highlight matching brackets")
        layout.addWidget(self.bracket_match_checkbox)

        # Autosave
        self.autosave_checkbox = QCheckBox("Enable autosave")
        self.autosave_checkbox.setChecked(True)
        self.autosave_checkbox.setToolTip("Automatically save files periodically")
        self.autosave_checkbox.stateChanged.connect(self._on_autosave_toggled)
        layout.addWidget(self.autosave_checkbox)

        # Autosave interval
        autosave_interval_widget = QWidget()
        autosave_interval_layout = QHBoxLayout(autosave_interval_widget)
        autosave_interval_layout.setContentsMargins(0, 0, 0, 0)
        autosave_interval_layout.setSpacing(8)

        autosave_interval_label = QLabel("Autosave interval:")
        autosave_interval_label.setFixedWidth(120)
        ...
        self.autosave_interval_spin.setFixedHeight(28)
        self.autosave_interval_spin.setToolTip("Minutes between automatic saves")
        autosave_interval_layout.addWidget(self.autosave_interval_spin)
        autosave_interval_layout.addStretch(1)

        layout.addWidget(autosave_interval_widget)

        # Info label
        info_label = QLabel(
            "💡 Editor settings apply to all code editors. Changes take effect immediately after saving."
        )
        info_label.setStyleSheet(SECTION_INFO_LABEL_STYLE)
        info_label.setWordWrap(True)
        layout.addWidget(info_label)
    ...
    def _on_autosave_toggled(self, state):
        """Enable/disable autosave interval based on autosave checkbox."""
        enabled = state == Qt.Checked
        if hasattr(self, 'autosave_interval_spin'):
    ...
    def show_success(self, title, message):
        ...
        error_service.show_success(title, message, self)
    ...
    def _save_and_close(self):
        ...
        try:
            ...
            # Emit signal first so listeners can reload
