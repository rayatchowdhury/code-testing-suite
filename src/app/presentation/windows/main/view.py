...
from src.app.presentation.shared.components.layout import DisplayArea
from src.app.presentation.shared.components.sidebar import Sidebar
from src.app.presentation.base import ContentWindowBase
...
class MainWindowContent(ContentWindowBase):
    ...
    def _show_options_dialog(self) -> None:
        ...
        try:
            ...
            # Connect signal to apply settings when saved
            config_dialog.configSaved.connect(self._on_config_saved)
            config_dialog.exec()
        except (ImportError, RuntimeError) as e:
            logger.error(f"Error opening options dialog: {e}", exc_info=True)
    
    def _on_config_saved(self, config: dict) -> None:
        """Handle configuration changes - apply immediately via hot reload (no restart required)."""
        try:
            ...
            logger.info("Configuration saved - applying hot reload")
            
            # Reload AI configuration
            try:
                reload_ai_config()
                logger.debug("AI config reloaded")
            except Exception as e:
                logger.warning(f"Could not reload AI config: {e}")
            
            # Apply to all windows via window manager
            if self.parent and hasattr(self.parent, 'window_manager'):
                window_manager = self.parent.window_manager
                if hasattr(window_manager, 'reload_all_configs'):
                    window_manager.reload_all_configs(config)
                    logger.debug("Window manager configs reloaded")
            
            # Notify all subscribers globally via ConfigChangeNotifier
            notifier = ConfigChangeNotifier.instance()
            notifier.notify_config_changed(config)
            
            logger.info("✓ Configuration applied successfully - no restart required!")
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event) -> None:
        ...
        try:
            # Check if tests are running in any window
            current_window = self.window_manager.get_current_window()
            if current_window and hasattr(current_window, '_is_test_running'):
                if current_window._is_test_running():
                    logger.warning("Tests are running - blocking app close")
                    from src.app.presentation.services import ErrorHandlerService
                    error_service = ErrorHandlerService.instance()
                    error_service.show_warning(
                        "Tests Running",
                        "Cannot close application while tests are running.\n\nPlease stop the current test execution first.",
                        self
                    )
                    event.ignore()
                    return
