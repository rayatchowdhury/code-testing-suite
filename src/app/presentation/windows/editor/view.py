...
class CodeEditorWindow(ContentWindowBase):
    ...
    def can_close(self):
        """Check if any tab has unsaved changes or if tests are running"""
        # First check parent (tests running, etc.)
    ...
    def save_editor_state(self):
        ...
        # Save to JSON file using FileOperations for consistency
        try:
            FileOperations.write_file(self.state_file, json.dumps(state, indent=2))
            logger.debug(f"Editor state saved to {self.state_file}")
        except (IOError, OSError) as e:
            logger.error(f"Failed to save editor state to file: {e}")
    ...
    def load_editor_state(self):
        ...
        try:
            ...
            # Use FileOperations for consistent file reading
            content = FileOperations.read_file(self.state_file)
            if not content:
                logger.warning("Editor state file is empty")
                return
