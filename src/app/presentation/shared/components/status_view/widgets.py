...
from src.app.presentation.shared.design_system.styles.components.status_view.status_widgets_styles import (
...
class TestResultsCardsSection(QWidget):
    """Dual-column test results with batched updates to prevent flickering"""
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self._pending_cards = []  # Queue for batched updates
        self._batch_timer = None  # Timer for deferred rendering
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        self.passed_scroll = QScrollArea()  # Store reference for update control
        self.passed_scroll.setWidgetResizable(True)
        self.passed_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.passed_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        self.passed_scroll.setStyleSheet(CARDS_SECTION_SCROLLBAR_STYLE)
        ...
        self.passed_scroll.setWidget(self.passed_cards_widget)
        passed_layout.addWidget(self.passed_scroll)
        ...
        self.failed_scroll = QScrollArea()  # Store reference for update control
        self.failed_scroll.setWidgetResizable(True)
        self.failed_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.failed_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        self.failed_scroll.setStyleSheet(CARDS_SECTION_SCROLLBAR_STYLE)
        ...
        self.failed_scroll.setWidget(self.failed_cards_widget)
        failed_layout.addWidget(self.failed_scroll)
    ...
    def _setup_batch_timer(self):
        """Setup single-shot timer for batched card rendering to prevent flickering"""
        self._batch_timer = QTimer(self)
        self._batch_timer.setSingleShot(True)  # Single-shot prevents re-entrancy
        self._batch_timer.setInterval(150)  # Adaptive: shorter interval, but single-shot
        self._batch_timer.timeout.connect(self._flush_pending_cards)
        self._is_flushing = False  # Re-entrancy guard
    
    def clear(self):
        """Clear all cards and pending queue"""
        # Stop batch timer and clear pending
        if self._batch_timer.isActive():
            self._batch_timer.stop()
        self._pending_cards.clear()
        
        # Disable updates during bulk clear to prevent flickering
    ...
    def add_card(self, card: QWidget, passed: bool):
        """Add test card with batched rendering to prevent flickering"""
        # Add to pending queue
        self._pending_cards.append((card, passed))
        
        # If currently flushing, cards will be picked up in next cycle
        if self._is_flushing:
            return
        
        # Start/restart single-shot timer
        if not self._batch_timer.isActive():
            self._batch_timer.start()
    
    def _flush_pending_cards(self):
        """Flush pending cards queue - ZERO-FLICKER implementation"""
        if not self._pending_cards or self._is_flushing:
            return
        
        # Set re-entrancy guard
        self._is_flushing = True
        
        # Track cards to show after batch is complete
        cards_to_show = []
        
        # Process all pending cards in one atomic operation
        for card, passed in self._pending_cards:
            # CRITICAL: Hide card to prevent individual repaints
            card.hide()
        ...
        # CRITICAL: Show all cards at once - this creates ONE paint event
        # instead of N individual paint events (one per card)
        ...
        # If more cards were added during flush, schedule next flush
        if self._pending_cards and not self._batch_timer.isActive():
            self._batch_timer.start()
