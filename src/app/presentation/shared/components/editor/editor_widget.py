...
from src.app.presentation.shared.design_system.styles.components.ai_panel import AI_PANEL_STYLE
...
from src.app.presentation.shared.design_system.tokens.editor_colors import EDITOR_COLORS
from src.app.shared.utils.file_operations import FileOperations
from src.app.presentation.shared.components.editor.component_loader import (
    ComponentLoader,
...
class AIWorkerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Catch-all for unexpected AI errors after specific error handling above
            # This is acceptable as AI errors can be unpredictable
            import traceback
...
class CodeEditor(QPlainTextEdit):
    def __init__(self, component_loader: ComponentLoader = None):
        super().__init__()
        self._loader = component_loader or get_component_loader()
        self._setup_editor()
    ...
    def _load_editor_settings(self):
        """Load editor settings from config file"""
        try:
            from src.app.core.config.core import ConfigManager
            # Use singleton instance for consistent config access
            config_manager = ConfigManager.instance()
            config = config_manager.load_config()
            editor_settings = config.get("editor_settings", {})
            logger.debug(f"Loaded editor settings: font={editor_settings.get('font_family')}, size={editor_settings.get('font_size')}")
            ...
            self.tab_spaces = editor_settings.get("tab_width", 4)
            self.auto_indent = True
            self.bracket_matching = editor_settings.get("bracket_matching", True)
            
            # Apply line wrap setting
            wrap_lines = editor_settings.get("wrap_lines", False)
            if wrap_lines:
                self.setLineWrapMode(QPlainTextEdit.WidgetWidth)
            else:
                self.setLineWrapMode(QPlainTextEdit.NoWrap)
        except (ImportError, AttributeError) as e:
            # Config module not available or incomplete
            ...
            logger.exception(f"Unexpected error loading editor settings: {e}")
            self.setFont(QFont("Consolas", 12))
            self.tab_spaces = 4
            self.auto_indent = True
            self.bracket_matching = True
    ...
    def _setup_syntax_highlighting(self, file_path=None):
        ...
        # Lazy load syntax highlighters via ComponentLoader
        highlighters = self._loader.syntax_highlighters
        highlighter_map = {
...
class EditorWidget(QWidget):
    ...
    def _subscribe_to_config_changes(self):
        """Subscribe to global config changes for hot reload."""
        try:
            ...
            pass  # Fail silently if notifier not available
    
    def _on_config_changed(self, config: dict):
        """Handle config changes from hot reload."""
        editor_settings = config.get("editor_settings", {})
        if editor_settings:
            self.apply_settings(editor_settings)
    ...
    def _setup_ui(self):
        ...
        # Add editor first
        self.codeEditor = CodeEditor(self._loader)
    ...
    def _init_ai_panel(self):
        ...
        if not self._ai_panel_initialized:
            ai_panel_class = self._loader.ai_panel
    ...
    def _get_editor_ai(self):
        ...
        if self.editor_ai is None:
            editor_ai_class = self._loader.editor_ai
    ...
    def apply_settings(self, editor_settings):
        """Apply editor settings to the code editor"""
        if self.codeEditor:
            self.codeEditor.apply_settings(editor_settings)
    ...
    def _show_ai_response(self, response: str, title: str):
        ...
        # Lazy load markdown and pygments via ComponentLoader
        markdown_func = self._loader.markdown
        highlight, formatter_class, lexer = self._loader.pygments
    ...
    def _process_ai_request(self, action, title):
        ...
        logger.info(f"Processing AI request: {title}")
    ...
    def _on_ai_response_ready(self, response: str, title: str):
        ...
        logger.info(f"AI response received for: {title}")
        self._show_ai_response(response, title)

    def _on_ai_error(self, error_message: str, title: str):
        """Handle AI error"""
        logger.error(f"AI error for {title}: {error_message}")
    ...
    def openFile(self, file_path):
        ...
        if os.path.exists(file_path):
            ...
            if content is not None:
                ...
                logger.error(f"Failed to load file content: {file_path}")
        else:
