...
class TestTabWidget(QWidget):
    ...
    def _ensure_multi_language_config(self):
        ...
        for tab_name, config in self.tab_config.items():
            ...
            self.unsaved_changes_per_tab[tab_name] = {
                lang: False for lang in self.available_languages
            }
    ...
    def _handle_language_change(self, tab_name, new_language):
        ...
        if (
            old_language
            and old_language != new_language
            and self.unsaved_changes_per_tab[tab_name].get(old_language, False)
        ):
    ...
    def _update_tab_language_indicator(self, tab_name, language):
        ...
        try:
            ...
            pass  # Button doesn't have language_label (not multi-language mode)
        ...
        if self.current_button:
            ...
            if self.multi_language:
                try:
                    ...
                    pass  # Button doesn't have tab_container
        ...
        if self.multi_language:
            try:
                ...
                has_unsaved = self.unsaved_changes_per_tab.get(tab_name, {}).get(
                    self.current_language_per_tab.get(tab_name, self.default_language),
                    False,
                )
    ...
    def _get_default_content(self, tab_name, language="cpp"):
        ...
        Reads template files from src/app/resources/templates directory.
        """
        ...
        extension_map = {"cpp": ".cpp", "py": ".py", "java": ".java"}
        ...
        # Navigate from shared/components/editor to app/resources/templates
        ...
        if tab_name in self.file_buttons:
            ...
            if self.multi_language:
                try:
                    ...
                    # Button doesn't have tab_container (fallback to single language mode)
    ...
    def has_unsaved_changes(self):
        ...
        if self.current_button:
            if self.multi_language:
                ...
                return self.unsaved_changes_per_tab.get(current_tab_name, {}).get(
                    current_lang, False
                )
    ...
    def get_all_file_paths_with_languages(self):
        ...
        for tab_name in self.tab_config.keys():
            ...
            result[tab_name] = {"language": current_lang, "file_path": file_path}
