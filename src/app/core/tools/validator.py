...
class ValidatorRunner(BaseRunner):
    ...
    )  # test number, passed, input, test_output, validation_message, error_details, validator_exit_code, exec_time, memory
    ...
    def _create_test_result(
        ...
        validation_analysis = {
            ...
            "execution_times": {
                ...
                "avg_test": (
                    sum(r.get("test_time", 0) for r in test_results) / len(test_results)
                    if test_results
                    else 0
                ),
                "avg_validator": (
