...
class Benchmarker(BaseRunner):
    ...
    def _create_test_worker(self, test_count, time_limit=None, memory_limit=None, max_workers=None, **kwargs):
    ...
    def _create_test_result(self, all_passed, test_results, passed_tests, failed_tests, total_time):
        ...
        benchmark_analysis = {
            ...
            'performance_summary': {
                ...
                'time_limit_exceeded': sum(1 for r in test_results if not r.get('time_passed', True)),
                'memory_limit_exceeded': sum(1 for r in test_results if not r.get('memory_passed', True)),
                'runtime_errors': sum(1 for r in test_results if r.get('error_details', '').startswith('Runtime Error')),
                'system_errors': sum(1 for r in test_results if 'Execution error' in r.get('error_details', ''))
            },
            'performance_metrics': {
                'avg_execution_time': sum(r.get('execution_time', 0) for r in test_results) / len(test_results) if test_results else 0,
                'max_execution_time': max((r.get('execution_time', 0) for r in test_results), default=0),
                'avg_memory_usage': sum(r.get('memory_used', 0) for r in test_results) / len(test_results) if test_results else 0,
