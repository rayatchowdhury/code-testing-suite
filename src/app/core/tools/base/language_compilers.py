...
class BaseLanguageCompiler(ABC):
    ...
    def compile(self, source_file: str, output_file: Optional[str] = None,
                custom_flags: Optional[List[str]] = None, timeout: int = 30) -> Tuple[bool, str]:
        """
        Compile source file.
...
class CppCompiler(BaseLanguageCompiler):
    ...
    def compile(self, source_file: str, output_file: Optional[str] = None,
        ...
        if custom_flags:
            ...
            default_flags = self.config.get('flags', ['-march=native', '-mtune=native', '-pipe', '-Wall'])
        ...
        try:
            ...
            return False, f"Compilation timeout ({timeout}s) for {os.path.basename(source_file)}"
        except FileNotFoundError:
            return False, f"Compiler '{compiler}' not found. Please install g++ or update configuration."
        except Exception as e:
...
class JavaCompiler(BaseLanguageCompiler):
    ...
    def compile(self, source_file: str, output_file: Optional[str] = None,
        ...
        try:
            ...
            return False, f"Compiler '{compiler}' not found. Please install Java JDK or update configuration."
        except Exception as e:
            return False, f"Compilation error: {str(e)}"
...
class LanguageCompilerFactory:
    ...
    @staticmethod
    def create_compiler(language: Language, config: Optional[Dict[str, Any]] = None) -> BaseLanguageCompiler:
        """
