...
def generate_report(categories: Dict) -> str:
    ...
    report.append(f"- **Files <70% Coverage**: {files_below_70} ({files_below_70/total_files*100:.1f}%)")
    report.append(f"- **Files <50% Coverage**: {files_below_50} ({files_below_50/total_files*100:.1f}%)")
    report.append(f"- **Files <30% Coverage**: {files_below_30} ({files_below_30/total_files*100:.1f}%)")
    report.append("")
    ...
    for category, files in sorted(categories.items()):
        ...
        layer_stats.append((category, avg_cov, len(files), below_70, below_50, below_30))
        report.append(f"| {category.replace('_', ' ').title()} | {avg_cov:.1f}% | {len(files)} | {below_70} | {below_50} | {below_30} |")
    ...
    critical_files.sort(key=lambda x: (x[2], -x[4]))  # Sort by coverage (asc), then by size (desc)
    ...
    if critical_files:
        ...
        for category, filepath, cov, missing, total in critical_files:
            ...
            report.append(f"  - **Action**: Add {(total - (total * cov / 100)) / 10:.0f}-{(total - (total * cov / 100)) / 5:.0f} tests")
            report.append("")
    else:
    ...
    if moderate_files:
        ...
        for category, filepath, cov, missing, total in moderate_files:
            ...
            report.append(f"- **`{filepath}`** - {cov:.1f}% ({missing}/{total} missing)")
            report.append(f"  - **Action**: Add {missing / 10:.0f}-{missing / 5:.0f} tests to reach 80%+")
            report.append("")
    else:
    ...
    report.append(f"   - Current avg: {sum(cov for _, cov, _, _ in categories.get('presentation_views', [])) / max(len(categories.get('presentation_views', [])), 1):.1f}%")
    report.append("   - Target: 50%+ (add ~30-40 tests)")
    report.append("   - Priority files: config_dialog.py, results windows, window controllers")
    report.append("")
    
    report.append("2. **Fill Core Tools Gaps**")
    report.append(f"   - Current avg: {sum(cov for _, cov, _, _ in categories.get('core_tools', [])) / max(len(categories.get('core_tools', [])), 1):.1f}%")
    ...
    report.append(f"| Priority 1 (<50%) | {priority_1_count} | {priority_1_count * 10}-{priority_1_count * 15} | 6-8 hours |")
    report.append(f"| Priority 2 (50-70%) | {priority_2_count} | {priority_2_count * 5}-{priority_2_count * 8} | 4-6 hours |")
    report.append(f"| Priority 3 (70-90%) | {priority_3_count} | {priority_3_count * 3}-{priority_3_count * 5} | 2-3 hours |")
    report.append(f"| **Total** | **{priority_1_count + priority_2_count + priority_3_count}** | **{priority_1_count * 10 + priority_2_count * 5 + priority_3_count * 3}-{priority_1_count * 15 + priority_2_count * 8 + priority_3_count * 5}** | **12-17 hours** |")
    report.append("")
...
def main():
    ...
    print(f"   Goal Status: {'✅ ACHIEVED' if avg_coverage >= 70 else f'🔄 {70 - avg_coverage:.1f}% to go'}")
